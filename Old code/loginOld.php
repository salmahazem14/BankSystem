...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$email || !$password || !$userType) {
        ...
        exit();
    } elseif (!in_array($userType, ['user', 'admin'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $user['password'])) {
            ...
            $action = "User with email $email logged in successfully.";
            $logStmt = $conn->prepare("INSERT INTO loggings (action) VALUES (?)");
            ...
            // Log failed attempt
            $action = "Failed login attempt for email $email from IP $ipAddress due to incorrect password.";
            $logStmt = $conn->prepare("INSERT INTO loggings (action) VALUES (?)");
            $logStmt->bind_param('s', $action);
            $logStmt->execute();
        ...
        // Log failed attempt
        $action = "Failed login attempt for email $email from IP $ipAddress due to no matching user.";
        $logStmt = $conn->prepare("INSERT INTO loggings (action) VALUES (?)");
        $logStmt->bind_param('s', $action);
        $logStmt->execute();
...
<body style="background: #f4f4f4; display: flex; justify-content: center; align-items: center; height: 100vh; color: #333; text-align: center; padding: 20px;">
    <div style="background: white; padding: 30px; border-radius: 12px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.1); width: 100%; max-width: 400px;">
        ...
        <form id="loginForm" method="POST" style="display: flex; flex-direction: column; gap: 15px; text-align: left;">
